...
impl_op_gen_1!(Mul, mul, f32, Vector<N>, Vector<N>, |a: f32, b: Vector<N>| Vector(a * b.0));
...
impl_op_gen_2!(Mul, mul, f32, Matrix<N, M>, Matrix<N, M>, |a: f32, b: Matrix<N, M>| Matrix(a * b.0));
impl_op_gen_2!(Mul, mul, Matrix<N, M>, Vector<M>, Vector<N>, |a: Matrix<N, M>, b: Vector<M>| Vector((a.0 * b.0).map(|v| {
    if v.abs() < 1.0e-5 { 
        0.0 
    } else { 
        v 
